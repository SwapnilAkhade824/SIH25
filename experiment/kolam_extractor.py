#!/usr/bin/env python3
"""
Kolam Feature Extractor - OpenCV Implementation
SIH 2025 Problem Statement ID 25107

A complete OpenCV-based solution for analyzing traditional Indian Kolam patterns
without requiring machine learning model training.
"""
...
class KolamFeatureExtractor:
    ...
    def _detect_grid_structure(self):
        ...
        if x_diffs and y_diffs:
            x_cv = np.std(x_diffs) / np.mean(x_diffs)
            y_cv = np.std(y_diffs) / np.mean(y_diffs)
    ...
    def generate_report(self):
        ...
        if 'geometric_patterns' in self.features:
            ...
            report.append(f"  Dominant Angles: {geom['dominant_angles'][:2]}")
